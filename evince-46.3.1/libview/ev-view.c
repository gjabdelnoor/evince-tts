...
/*** Drawing ***/
static void       highlight_tts_sentence                     (EvView             *view,
							      cairo_t            *cr,
							      int                 page);
              ...
              cairo_t   *cr)
{
	...
	for (i = view->start_page; i >= 0 && i <= view->end_page; i++) {
		...
		if (page_ready && view->tts_n_rects > 0 && view->tts_page == i)
			highlight_tts_sentence (view, cr, i);
...
static void
highlight_tts_sentence (EvView  *view,
                        cairo_t *cr,
                        int      page)
{
	EvRectangle *ev_rect = ev_rectangle_new ();

	for (guint i = 0; i < view->tts_n_rects; i++) {
		GdkRectangle view_rectangle;

		*ev_rect = view->tts_rects[i];
		_ev_view_transform_doc_rect_to_view_rect (view, page, ev_rect, &view_rectangle);
		draw_rubberband (view, cr, &view_rectangle, TRUE);
	}
...
ev_view_finalize (GObject *object)
{
	...
	g_clear_pointer (&view->synctex_result, g_free);
	g_clear_pointer (&view->tts_rects, g_free);
...
/**
 * ev_view_set_tts_highlight:
 * @view: an #EvView
 * @page: the page the rectangles refer to
 * @rects: (array length=n_rects) (nullable): doc-space rectangles to highlight
 * @n_rects: number of rectangles
 *
 * Highlights the currently spoken sentence. Pass @rects = %NULL (or
 * @n_rects = 0) to clear. Scrolls the first rectangle into view.
 */
void
ev_view_set_tts_highlight (EvView            *view,
                           gint               page,
                           const EvRectangle *rects,
                           guint              n_rects)
{
	g_return_if_fail (EV_IS_VIEW (view));
	...
	if (rects && n_rects > 0) {
		view->tts_rects = g_memdup2 (rects, n_rects * sizeof (EvRectangle));
		view->tts_n_rects = n_rects;

		if (view->document) {
			GdkRectangle view_rect;
			EvRectangle r = rects[0];
			_ev_view_transform_doc_rect_to_view_rect (view, page, &r, &view_rect);
			_ev_view_ensure_rectangle_is_visible (view, &view_rect);
		}
	}
