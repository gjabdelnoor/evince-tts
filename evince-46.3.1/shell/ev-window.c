...
static void
ev_window_tts_status_cb (EvWindow *window, const char *msg)
{
	EvWindowPrivate *priv = GET_PRIVATE (window);

	g_message ("TTS: %s", msg);
	/* Surface actionable/terminal messages (config missing, errors, finished)
	 * in the info bar; suppress transient per-sentence progress while active. */
	if (!ev_tts_controller_is_active (priv->tts_controller))
		ev_window_warning_message (window, "%s", msg);
}

static void
ev_window_tts_active_cb (EvWindow *window)
{
	EvWindowPrivate *priv = GET_PRIVATE (window);
	GAction  *action;
	gboolean  active = ev_tts_controller_is_active (priv->tts_controller);

	action = g_action_map_lookup_action (G_ACTION_MAP (window), "read-aloud");
	if (action)
		g_simple_action_set_state (G_SIMPLE_ACTION (action),
...
static void
ev_window_cmd_toggle_read_aloud (GSimpleAction *action,
				 GVariant      *state,
				 gpointer       user_data)
{
	EvWindow *window = user_data;
...
static void
ev_window_cmd_tts_preferences (GSimpleAction *action,
			       GVariant      *parameter,
			       gpointer       user_data)
{
	EvWindow *window = user_data;
...
ev_window_init (EvWindow *ev_window)
{
	...
	priv->tts_controller = ev_tts_controller_new (EV_VIEW (priv->view), priv->model);
	g_signal_connect_swapped (priv->tts_controller, "status",
